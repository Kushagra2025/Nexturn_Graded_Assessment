...
func (repo *ProductRepository) CreateProduct(product *model.Product) (*model.Product, error) {
	stmt, err := repo.DB.Prepare("INSERT INTO products (name, price, quantity) VALUES (?, ?, ?)")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()

	res, err := stmt.Exec(product.Name, product.Price, product.Quantity)
	if err != nil {
		return nil, err
	}
...
func (repo *ProductRepository) UpdateProduct(product *model.Product) (*model.Product, error) {
	stmt, err := repo.DB.Prepare("UPDATE products SET name = ?, price = ?, quantity = ? WHERE id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
